module Erlen
  ...
  module ControllerHelper
    ...
    module ClassMethods
      ...
      def request_schema(action, request)
        __erlen__create_before_action(action, request)
        nil
      end

      def __erlen__create_before_action(action, request_schema)
        define_method(:"validate_request_schema_for_#{action}") do
          ...
          return if @request_payload.valid?
    ...
    # This contains the current action's request payload
